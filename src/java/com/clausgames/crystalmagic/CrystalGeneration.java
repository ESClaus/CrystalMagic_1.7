...
public class CrystalGeneration implements IWorldGenerator {
	...
	public void generateNether(World world, Random rand, int x, int z){ //Dimension ID -1
		
	}
	public void generateOverworld(World world, Random rand, int x, int z){ //Dimension ID 0
		generateOre(ModBlocks.blockCrystal, world, rand, x, z, 4, 8, 10, 0, 100, Blocks.stone); //Look at generateOre method below
	}
	public void generateEnd(World world, Random rand, int x, int z){ //Dimension ID 1
		
	}
	public void generateOre(Block block, World world, Random random, int chunkX, int chunkZ, int minVeinSize, int maxVeinSize, int chance, int minY, int maxY, Block generateIn){ //Block we are spawning, world, rand, x, z, min Vein, Max Vein, chance it will occur, y level min, y level max, what block types it generates in (stone, grass/dirt, etc).
