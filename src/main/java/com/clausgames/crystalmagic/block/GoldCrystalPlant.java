...
public class GoldCrystalPlant extends OreCrystalPlant
{
    ...
    public boolean canPlaceBlockAt(World worldIn, int x, int y, int z) {
        return worldIn.getBlock(x, y, z).isReplaceable(worldIn, x, y, z) && worldIn.getBlock(x, y + 1, z) == ModBlocks.goldInfusedStone; //The Soil Block
    }

    @Override
    public boolean canBlockStay(World worldIn, int x, int y, int z) {
        return worldIn.isAirBlock(x, y - 1, z) && worldIn.getBlock(x, y + 1, z) == ModBlocks.goldInfusedStone && worldIn.getBlock(x, y + 2, z).getMaterial() == Material.water; //Is block above where I click of type water and soil block?
    }
